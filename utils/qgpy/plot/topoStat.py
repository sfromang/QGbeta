...
def getMeanVar(istart,istop,type='psi',ghost=False,verbose=True):
    ...
    for idump in range(istart,istop+1):
        ...
        if (idump==istart):
            ...
            varMean=varMean+data.getVar(type=type,ilevel=0,ghost=ghost)
...
def defineZone(xvals,yvals):
    """ Define light gray area... """
    plt.plot([xvals[0],xvals[0]],yvals,':k',linewidth=1)
    plt.plot([xvals[1],xvals[1]],yvals,':k',linewidth=1)
    plt.fill_between(xvals,[yvals[0],yvals[0]],[yvals[1],yvals[1]],color='lightgray')
    return
