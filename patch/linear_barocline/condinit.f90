...
subroutine condinit
  ...
  k=2.*pi/Lx
  xi=(2.d0*LambdaInvSq-k**2)/(2.d0*LambdaInvSq+k**2)
  ...
  if (randomInit) then
     ...
     do i=0,nx+1
        do ilayer=1,2
           ...
           psi(i,j,ilayer)=psibar(i,j,ilayer)+maxPsibar*amp*(rvalue-0.5)
        end do
     end do
  else
     ! Sinusoidal perturbations on perturbed streamfunction
     do i=0,nx+1
        psi(i,:,1)=cos(k*x(i))-xi**0.5*sin(k*x(i))
        psi(i,:,2)=cos(k*x(i))+xi**0.5*sin(k*x(i))
     end do
     psi=psibar+maxPsibar*amp*psi
  endif
